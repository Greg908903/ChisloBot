...
def starting(message):
    set_dics(id, message.from_user.first_name + ' ' + message.from_user.last_name)
...
def print_table_res(message):
    ...
    if message.chat.id not in flag_run:
        set_dics(message.chat.id, message.from_user.first_name + ' ' + message.from_user.last_name)
...
def check_answer(message):
    ...
    if flag_run[id] == 1:
        ...
        if answer_for_task[id] == message.text:
            combo[message.from_user.first_name + ' ' + message.from_user.last_name] += 1
            if combo[message.from_user.first_name + ' ' + message.from_user.last_name] \
                    > records[message.from_user.first_name + ' ' + message.from_user.last_name]:
                records[message.from_user.first_name + ' ' + message.from_user.last_name] \
                    = combo[message.from_user.first_name + ' ' + message.from_user.last_name]
            result[id] += 1
            glob_results[message.from_user.first_name + ' ' + message.from_user.last_name] += 1
            result_max[id] += 1
            bot.send_message(id,
                             f'Правильно! Уже {result[id]} {morph.parse("правильный")[0].make_agree_with_number(result[id]).word} {morph.parse("ответ")[0].make_agree_with_number(result[id]).word}.')
            generate_task(message)
            time.sleep(1)
            bot.send_message(id, task[id])
        else:
            result_max[id] += 1
            combo[message.from_user.first_name + ' ' + message.from_user.last_name] = 0
